...
$(window).ready(function () {
    ...
    $(".start").click(function () {
        ...
        $("#instructions").hide();
        $("#playing").show();
        var rFilter = new createjs.ColorFilter(1, 0, 0, 1, 0, 0, 0, 0);
        var gbFilter = new createjs.ColorFilter(0, 1, 1, 1, 0, 0, 0, 0);
        if (window.glasses == 0) {
            window.colors = ["r", "gb"];
            window.filters = [rFilter, gbFilter];
        } else {
            window.colors = ["gb", "r"];
            window.filters = [gbFilter, rFilter];
        }
        Logic.initialize();
        //init();
    });
